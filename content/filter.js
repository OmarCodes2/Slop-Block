...
window.LinkedInFilter.countPositiveCategories = function(text) {
  ...
  if (window.LinkedInFilter.POSITIVE_SIGNALS.RECRUITER_IDENTITY.some(signal => lowerText.includes(signal))) {
    categoryCount++;
  }

  // Check call-to-action signals
  if (window.LinkedInFilter.POSITIVE_SIGNALS.CALL_TO_ACTION.some(signal => lowerText.includes(signal))) {
    categoryCount++;
  }

  // Check role listing signals
  if (window.LinkedInFilter.POSITIVE_SIGNALS.ROLE_LISTING.some(signal => lowerText.includes(signal))) {
    categoryCount++;
  }
  ...
  const hasCompensation = window.LinkedInFilter.POSITIVE_SIGNALS.STRUCTURAL.some(signal => lowerText.includes(signal));
...
window.LinkedInFilter.applyLocalHeuristics = function(text) {
  ...
  const positiveCategoryCount = window.LinkedInFilter.countPositiveCategories(text);
 ...
 */
window.LinkedInFilter.classifyWithAI = function(postElement, postText, postKey, pendingClassifications, onClassificationResult) {
